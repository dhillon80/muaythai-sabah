...
export default function NewsletterArchive() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-gray-200 pt-24 md:pt-32 pb-12 md:pb-20 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        
        {/* --- HEADER --- */}
        <header className="mb-12 md:mb-20">
          <h1 className="text-4xl md:text-7xl font-black text-white uppercase italic tracking-tighter mb-4 leading-none">
            MUAYTHAI SABAH <span className="text-yellow-500 underline decoration-red-600 underline-offset-4 md:underline-offset-8">NEWSLETTER</span>
          </h1>
          <p className="text-gray-400 text-sm sm:text-lg md:text-xl font-bold italic uppercase tracking-widest mb-8 md:mb-10">
          ...
          <div className="flex flex-wrap gap-3 md:gap-4">
            {["2026", "2025"].map(year => (
              <button 
                ...
                className={`px-6 py-3 sm:px-10 sm:py-4 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-black uppercase tracking-widest italic transition-all border ${
                  activeYear === year 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {filteredStories.map((story, i) => (
            <Link key={i} href={story.link} className="group flex flex-col bg-slate-900/40 border border-white/5 rounded-3xl md:rounded-[2.5rem] overflow-hidden hover:bg-slate-900 transition-all hover:-translate-y-2 shadow-2xl">
              ...
              <div className="p-6 md:p-8 flex-1 flex flex-col justify-between">
                <div>
                  <span className="text-[9px] font-black text-yellow-500 uppercase tracking-widest mb-2 md:mb-3 block italic">{story.tag}</span>
                  <h2 className="text-lg md:text-xl font-black text-white uppercase italic leading-tight group-hover:text-yellow-500 transition-colors">
                    {story.title}
                  </h2>
                </div>
